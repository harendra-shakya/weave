 ...
 * a SIMULATED event and are never really executed. Returns the refreshed Home so
 * the UI can reflect the new state immediately (and after restart, from disk).
 */
...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ ok: true, simulated: true, home });
}
